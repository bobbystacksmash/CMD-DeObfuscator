...
function HANDLE_for (identcmd, context) {
    // EXAMPLE:
    //
    //   FOR /L %G IN (1,1,5) DO echo %G
    let loop_tokens = identcmd.rest;

    // Loop type identification
    //
    //  FOR /L %G IN (1,1,5) DO echo %G
    //      ^^
    // First, we check to see what /type/ of loop this is by parsing
    // the first token which *should* be a switch:
    //
    if (!is(loop_tokens[0], ["STRING_DQUOTE", "LITERAL"])) {
        throw new Error("Syntax error: unknown loop switch flag: " + loop_tokens[0].text);
    }

    let loop_type = null;
    switch(loop_tokens[0].text.toLowerCase().replace(/^\"|\"$/g, "")) {
    case "/l":
        loop_type = "LIST_OF_NUMBERS";
        break;
    default:
        throw new Error("Syntax error: unknown loop switch flag: " + loop_tokens[0].text);
    }

    // Introduce a new loop var
    //
    //  FOR /L %G IN (1,1,5) DO echo %G
    //         ^^
    // The introduced variable will be used as a pointer to the
    // current loop element.

    // TODO: write an util function which strips leading "DELIMITERS"
    // from a token stream.
