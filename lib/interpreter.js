...
function FILTER_strip_escapes (tokens) {
    ...
    return tokenise(tokens.map(x => x.text).join(""));
}
...
function HANDLE_set (identcmd, context) {
    ...
    let cmd_rest = cmd_head_rest(identcmd.rest),
        tokens   = tokenise(`set ${cmd_rest.head.map(x => x.text).join("")}`).slice(2);

    // We re-tokenise the input because the presence of any escapes
    // may cause the command to be incorrectly ignored.
    let line = tokens.map(x => x.text).join("");

    context[0].commands.push({
        command: { name: "set", line: line },
        options: {}
    });
