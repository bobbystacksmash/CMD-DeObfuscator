...
function HANDLE_set (identcmd, context) {
    ...
    context.push({
        command: { name: "set", line: identcmd.rest.map(x => x.text).join("") },
        options: {},
        variables: {}
    });
    ...
    for (let i = 0; i < identcmd.rest.length; i++) {
        let token = identcmd.rest[i];
        if (is(token, "SET_VAR")) {
            _var = token.text;
        }
        else if (is(token, "SET_VALUE")) {
            _val = token.text;
            if (!has(context[0], "variables")) {
                context[context.length - 1].options = {};
            }
            context[context.length - 1].variables[_var.toLowerCase()] = _val;
        }
    }
}
function HANDLE_default (identcmd, context) {
    context.push({
        command: { name: identcmd.command, line: identcmd.rest.map(x => x.text).join("") },
        options: {},
        variables: {}
    });
}
...
function HANDLE_cmd (identcmd, context) {
    ...
    function parse_switches (tokens) {
    ...
    let switchobj = parse_switches(identcmd.rest),
        nextcmd   = switchobj.rest;

    if (context.length === 0) {
        context.push({
            command: {
                name: identcmd.command,
                line: identcmd.rest.map(x => x.text).join("")
            },
            options: {},
            variables: {}
        });
    ...
    // Add any parsed opts in to the current context.
    context[context.length - 1].options =
        Object.assign({}, context[context.length - 1].options, switchobj.switches);
...
function try_identify_command (tokens) {
    ...
    let original = tmptokens.map(t => t.text).join(""),
        ...
        output   = { command: null, rest: [] };

    if (/^[a-z]:/i.test(clean) || /^[.]{1,2}[\\/]/.test(clean)
        || /^[\\/]/.test(clean)
        || (!/^\w+ /i.test(clean) && /[\\/][a-z0-9]+(?:\.[a-z0-9]+)?$/i.test(clean))) {

        output.command  = path.basename(clean).replace(/\.(?:exe|com)$/, "").toLowerCase();
        output.rest = resttokens;
    }
    else {
        output.command  = clean.split(/ /)[0].replace(/\.[a-z]+$/, "");
        output.rest = resttokens;
    }
